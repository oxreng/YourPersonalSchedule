...
def tasks():
    session = db_session.create_session()
    active_tasks = sorted(session.query(Task).filter(Task.active is True and Task.user_id == current_user.id).all(),
                          key=lambda x: x.date_start)
    inactive_tasks = sorted(session.query(Task).filter(Task.active is False and Task.user_id == current_user.id).all(),
                            key=lambda x: x.date_start)
